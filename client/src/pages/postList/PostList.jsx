...
export default function PostList() {
      ...
      renderCell:(params)=>{
        return (
          <div className='postList'>
            <img src={`${params.row.image}`} alt="" className="postListImg" />
          </div>
        )
      }
    },
  ...
  const deleteContact = async (params) => {
    ...
    if(window.confirm("Are you sure that you wanted to delete that contact ?")) {
       await axios.delete(`https://admin-23ei.onrender.com/post/${id}`, { data: { image } });
  ...
  const loadData = async () => {
    const response = await axios.get("https://admin-23ei.onrender.com/post");
